<div align="center">

# APP 02 PCDF

[![Instagram](https://img.shields.io/badge/Instagram-%40dupla09eps-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](#)

**Repositório da disciplina Engenharia de Produto de Software — 2026.1**  
**Universidade de Brasília — FCTE Gama**

</div>

---

## 📌 Sobre o repositório

Este repositório destina-se à documentação e ao desenvolvimento das atividades da disciplina **Engenharia de Produto de Software**, cursada no semestre **2026.1**, na **Universidade de Brasília — FCTE Gama**.

Também foi criado um Instagram para documentar as atividades diárias da dupla:

**@dupla09eps**

---

## 👥 Equipe

| Integrante | Matrícula / Identificação |
|------------|---------------------------|
| Carlos Eduardo Mota Alves | 221022248 |
| Samuel Alves | 202063462 |

---

## 🏫 Instituição

| Campo | Informação |
|------|------------|
| Universidade | Universidade de Brasília |
| Unidade | FCTE — Gama |
| Disciplina | Engenharia de Produto de Software |
| Semestre | 2026.1 |

---

## 🚀 Entregas iniciais

| Etapa | Descrição |
|------|-----------|
| App 02 | Primeira parte do trabalho desenvolvida pela dupla |
| Documentação | Organização dos artefatos do projeto |
| Estrutura do repositório | Separação dos materiais e evolução do projeto |

---

## 🎯 Objetivo

O objetivo deste repositório é centralizar os materiais, registros e artefatos produzidos ao longo da disciplina, garantindo organização, rastreabilidade e clareza durante o desenvolvimento do projeto.

---

## 🧩 Organização

A estrutura do repositório será utilizada para armazenar:

- documentação do projeto;
- artefatos produzidos pela equipe;
- registros de evolução das entregas;
- materiais de apoio da disciplina.

---

<div align="center">

**Dupla 09 — UnB FCTE Gama**

</div>
